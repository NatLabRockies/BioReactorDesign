...
def read_exp_data(experimental_data_folder:str|None=None)->dict:
    """
    Reads experimental data from a folder that contains a bunch of .dat
    Returns a dict that contains the experimental data
    """

    # Files to read from the exp data folder
    filenames = ["alpha-of-r_y1.dat", "alpha-of-r_y2.dat", "alpha-of-r_y3.dat", "vl-of-r_y2.dat", "vl-of-r_y1.dat", "vl-of-r_y3.dat"]
    key_names = [filename[:-4] for filename in filenames]

    # Dict that contains the experimental data
    exp_data = {}
    if experimental_data_folder is not None:
        for filename, key in zip(filenames, key_names):
            file = os.path.join(experimental_data_folder, filename)
            if not os.path.isfile(file):
                exp_data[key] = None
                logger.warning(f"Could not find {file}")
            else:
                exp_data[key] = np.genfromtxt(file, delimiter=",")
...
def make_plot(fields_cond: dict, heights: list[float], field_names=list[str], experimental_data_folder:str|None=None):
    ...
    for ifield, field_name in enumerate(field_names):
        for iheight, height in enumerate(heights):
            ...
            if field_name == "Ualpha.liquid":
                ax.plot(
                    fields_cond[field_name][height_name]["radius"]/0.03175,
                    fields_cond[field_name][height_name]["val"]/(1.0 - fields_cond["alpha.gas"][height_name]["val"]),
                    "bo",
                    #color="b",
                    #linewidth=3,
                    label="sim"
                ...
                pretty_labels(
                    "$r/R$",
                    "$\overline{\\alpha_l v_l}/\overline{\\alpha_l}$",
                    fontname="Times",
                    fontsize=16,
                    ax=ax,
                    grid=False,
